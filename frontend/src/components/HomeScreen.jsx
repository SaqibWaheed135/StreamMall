  ...
  const LiveRoomCard = ({ room }) => (
    <div className="group cursor-pointer relative">
      <div className="relative rounded-2xl overflow-hidden bg-black shadow-lg border border-white/10">
        ...
        />
        <div className="absolute top-3 left-3 flex items-center gap-2 bg-gradient-to-r from-[#FF2B55] to-[#7B2FF7] px-3 py-1.5 rounded-full backdrop-blur-sm">
          <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
          <span className="text-white text-xs font-bold">LIVE</span>
        </div>
        <div className="absolute top-3 right-3 bg-black/60 backdrop-blur px-3 py-1.5 rounded-full flex items-center gap-1">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-300 text-xs">{room.description}</p>
            </div>
          </div>
          ...
          {room.products?.length > 0 && (
            <div className="mb-3 flex gap-2 overflow-x-auto">
              {room.products.slice(0, 2).map((product) => (
                <div
                  key={product._id}
